...
\end{enumerate}
We pinpoint execution as the bottleneck (cf.\ \cref{sec:rep_analysis}), highlighting how using LLMs to generate code plans and then delegating to an external solver is the optimal policy among the 3 routes (\cref{sec:performance}). In the 40-task analysis set, Route~3 exceeds Route~2 by +31.47pp. The recovery mass where Route~2 succeeds while Route~3 fails is 1.61\%, whereas the execution-win mass where Route~3 succeeds while Route~2 fails is 33.08\%.
