...
\paragraph{Functional similarity condition.}
Let $E_{\mathrm{Tran}} := T\circ E_{\mathrm{Code}}$ be the NL-style trace
generator obtained by translating code traces into NL traces. Assume that
translated traces preserve downstream risk for the NL executor class:
\[
\sup_{\rho\in\mathcal H_{\mathrm{NL}}}
\left|
R(E_{\mathrm{NL}},\rho)-R(E_{\mathrm{Tran}},\rho)
\right|
\le \varepsilon.
\]
Assume also that the code-side executor class can realize translation followed
by any NL executor: for every $\rho\in\mathcal H_{\mathrm{NL}}$, the composed
executor $\rho\circ T$ belongs to $\mathcal H_{\mathrm C}$.

\begin{proposition}
Under the above conditions,
\[
R^*_{\mathcal H_{\mathrm C}}(E_{\mathrm{Code}})
\le
R^*_{\mathcal H_{\mathrm{NL}}}(E_{\mathrm{NL}})
+
\varepsilon.
\]
\end{proposition}
...
$\rho_\delta\in\mathcal H_{\mathrm{NL}}$ such that
\[
R(E_{\mathrm{NL}},\rho_\delta)
\le
R^*_{\mathcal H_{\mathrm{NL}}}(E_{\mathrm{NL}})+\delta.
\]
By realizability, the composed executor $\rho_\delta\circ T$ belongs to
$\mathcal H_{\mathrm C}$. Therefore,
\[
R^*_{\mathcal H_{\mathrm C}}(E_{\mathrm{Code}})
\le
R(E_{\mathrm{Code}},\rho_\delta\circ T).
\]
By construction, running $\rho_\delta\circ T$ on code traces has the same risk
as running $\rho_\delta$ on translated traces:
\[
R(E_{\mathrm{Code}},\rho_\delta\circ T)
=
R(E_{\mathrm{Tran}},\rho_\delta).
\]
By functional similarity,
\[
R(E_{\mathrm{Tran}},\rho_\delta)
\le
R(E_{\mathrm{NL}},\rho_\delta)+\varepsilon
\le
R^*_{\mathcal H_{\mathrm{NL}}}(E_{\mathrm{NL}})+\delta+\varepsilon.
\]
...
\begin{figure*}[t]
    \centering
    \vspace{-5pt}
    % \includegraphics[width=\textwidth]{images/translation_additivity.png}
    \includegraphics[width=\textwidth]{images/review_20260528/fig8_additivity.pdf}
    \vspace{-3pt}
